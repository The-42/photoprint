...
const char *MD5Digest::GetPrintableDigest()
{
	...
	for(int i=0;i<16;++i)
	{
		...
		b+='0';
		if(b>'9') b+=('a'-'9')-1;
		digestprintable[i*2]=a;
		digestprintable[i*2+1]=b;
	}
	digestprintable[32]=0;

	return(digestprintable);
}
...
std::ostream& operator<<(std::ostream &o, MD5Digest &d)
{
	const char *out=d.GetPrintableDigest();
	o << out;
	return(o);	
}
